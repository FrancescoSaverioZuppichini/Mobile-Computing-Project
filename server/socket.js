  ...
  io.on('connection', function (socket) {
    ...
    socket.on("identify_me", async(id) => {
      ...
      neighbors.forEach(async(neighbor) => {
        ...
        if (isOnline) {
          if (socketTemp.isAskingHelp) {
            var user = await User.findById(neighbor._id)
    ...
    socket.on("help_accepted", async(to) => {
      ...
      if (isOnline) {
        ...
        toSocket.emit("help_accepted_success", {
          from: user
        })
      }
    ...
    socket.on("help_stop_user", async(who) => {
      ...
      if (isOnline) {
        toSocket.emit("help_stop_user_success", {
          from: user
        })
      }
    })
